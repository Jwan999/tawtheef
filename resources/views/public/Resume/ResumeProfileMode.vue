<template>
    ...
    <div v-else :class="!isDashboard ? 'px-4 md:px-8 mt-6 mb-10' : ''">
        <div class="flex w-full relative space-x-0 md:space-x-6">
            ...
            <div class="md:w-3/12 relative">
                ...
                <button
                    ...
                    :class="[
                'md:hidden fixed top-16 z-50 bg-orange px-2 py-2 rounded-full shadow-lg transition-all duration-300 ease-in-out',
                mobileMenuOpen ? 'left-[232px]' : 'left-4'
            ]">
        ...
        'flex flex-col', 'pt-4 md:pt-0', 'overflow-y-auto']">
                    <div class="flex space-x-6 pb-6 border-b-[1px] border-zinc-200 mx-4 my-5">
<!--                        <div class="space-y-2 w-4/12">-->
<!--                            <img v-if="image" class="object-cover rounded-md h-20 w-full"-->
<!--                                 :src="`/storage/${image}`"-->
<!--                                 alt="">-->
<!--                            <div v-else class="flex justify-center bg-zinc-100 rounded-xl items-center py-3">-->
<!--                                <svg  class="w-14 fill-dark" viewBox="-42 0 512 512.002"-->
...
<!--                        </div>-->
                        <div class="space-y-3">
                            <h1 class="text-xl font-semibold tracking-wide capitalize">{{ contact.fullName }}</h1>
                            <PublishState v-if="isEditable" class="md:w-auto w-full mt-2" :isPublished="published"/>
                        </div>
                    </div>
