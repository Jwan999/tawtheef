<template>
    <div @click="goToResume(applicant.id)"
        ...
        <div class="relative aspect-[4/3] w-full overflow-hidden bg-zinc-100">
            <img v-if="applicant.image"
                 :src="getImageSrc"
                 class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-300"
                 alt="Applicant photo">
            <div v-else class="w-full h-full flex items-center justify-center">
                <svg class="w-1/2 h-1/2 text-zinc-400" viewBox="-42 0 512 512.002" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="p-6">
            ...
            <!-- Skills/Tools -->
            <div v-if="applicant.topTools?.length" class="mb-4">
                <div class="flex flex-wrap gap-2">
                    <span v-for="tool in applicant.topTools.slice(0, 3)"
                          :key="tool.item"
...
const getImageSrc = computed(() => {
    if (!props.applicant.image) return null;
    return props.applicant.image.startsWith('data:')
        ? props.applicant.image
        : `/storage/${props.applicant.image}`;
});
