<template>
    <div :class="[rounded, 'relative bg-white w-72 p-4 inline-block cursor-pointer rounded-lg']">
        <div @click="goToResume(applicant.id)" class="w-full">
            <div>
                ...
                <div class="flex w-full items-center mt-3">
                    <div class="border-[1px] border-orange p-1 rounded-3xl w-64">
                        <div v-if="applicant.image">
                            <img
                                v-if="applicant.image.startsWith('data:')"
                                class="h-44 object-cover w-full rounded-3xl"
                                :src="applicant.image"
                                alt=""
                            >
                            <img
                                v-else
                                class="h-44 rounded-3xl object-cover w-full"
                                :src="`/storage/${applicant.image}`"
                                alt=""
                            >
                        </div>
                        <div v-else class="flex justify-center items-center h-44 bg-zinc-100 rounded-3xl">
                            <svg class="h-32 fill-dark" viewBox="-42 0 512 512.002" xmlns="http://www.w3.org/2000/svg">
                                <path
                ...
                <div class="mt-2">
                    <div v-if="applicant.topTools && applicant.topTools.length > 0"
                         class="flex flew-wrap items-center space-x-2 capitalize">
...
const isTruncated = computed(() => {
    return props.applicant.summary.split(' ').length > wordLimit;
});
