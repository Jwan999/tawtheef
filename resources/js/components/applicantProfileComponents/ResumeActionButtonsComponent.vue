<template>
    <div>
        <div class="flex justify-end">
            <button v-if="editingAbility()" @click="editMode ? emit('saveResume') : toggleEditMode()"
            ...
            <button :class="editMode && editingAbility() ? '':'rounded-lb-md'"
                    class="appearance-none text-sm px-3 py-2 font-semibold text-zinc-500 bg-zinc-200">
...
import {getAuthUser, getSelectables} from "../../utils/storeHelpers.js";
...
const editingAbility = () => {
    ...
    const resumeRegex = /^\/resume\/(\d+)$/;
    const profileEditRegex = /^\/profile\/(\d+)\/edit$/;

    const resumeMatch = url.match(resumeRegex);
    const profileEditMatch = url.match(profileEditRegex);

    if (resumeMatch) {
        const id = resumeMatch[1];
        return id === String(userId.value);
    }
