    ...
    <div v-if="!editMode" class="rounded-md p-4 bg-white text-sm md:text-sm">
        <div class="space-y-1">
            ...
            <div v-else v-for="(tool,index) in value" :key="index" class="flex items-center justify-between">
                ...
                <span class="text-sm font-medium text-orange">{{ getCompetencyLevel(tool.rating) }}</span>
            </div>
        </div>
    </div>
    ...
    <div v-else class="rounded-md py-4 bg-white text-sm md:text-sm">
        <div class="text-sm md:text-sm">
            <div class="relative w-full">
                <div class="space-y-6">
                    <p class="block mb-3 text-base -mt-3">List all the tools and technologies you're familiar with such as (MS Excel, Photoshop, PHP, React, etc...)</p>
                    ...
                    <div v-for="(item,index) in value" :key="index">
                        ...
                        <ratingComponent placeholder-label="Ex: MS Excel"
                                         v-model="value[index]"
                                         ref="ratingComponents"></ratingComponent>
                    </div>
                </div>
...
const validateFields = () => {
    let isValid = true;
    ratingComponents.value.forEach(component => {
        if (component && typeof component.validateFields === 'function') {
            isValid = component.validateFields() && isValid;
        }
    });
    return isValid;
};
